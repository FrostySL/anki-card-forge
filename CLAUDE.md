...
- **`mirror`** (`python3 tools/anki_connect.py mirror [deck …]`): snapshots all
  ...
  committed.
- **`update-note <nid> --field "Name=<html>"`**: edits ONE note in place
  (updateNoteFields; deck backed up first). This is the ONLY way to change
  notes whose **note type was not built by this repo** — a `.apkg` push cannot
  update them (Anki skips same-GUID notes when the note type differs). Check
  the note type before a rework (mirror decode warns about foreign types);
