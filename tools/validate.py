...
def validate(apkg):
    ...
    # Anki can change the process directory while opening a collection.
    apkg = os.path.abspath(apkg)
    with tempfile.TemporaryDirectory(prefix="forge-validate-") as tmp:
        col = Collection(os.path.join(tmp, "col.anki2"))
        try:
            return _validate_collection(col, apkg)
        finally:
            # Windows cannot remove the temporary database while it is open.
            col.close()


def _validate_collection(col, apkg):
